...
def generate_graph(qfunc, action_values, int_rate=0.024, trans_cost=0.0005):
    spx_df = pd.read_csv('dataset/eval_spx.csv', index_col=0, parse_dates=True)
    ...
    ax1.plot(spx_df.index, spx_df['spx_normalised'], label='SPX', alpha=0.7)
    ...
    ax2.set_ylabel('Position (Allocation to SPX)')
    ax2.set_xlabel('Date')
